...
export function AiAssistant({ loading, onGenerate, lastCode, embedded = false }: AiAssistantProps) {
  const [text, setText] = useState(
    'Filtre les virements de plus de 5 000 €',
  );
  ...
  return (
    <section className={`ai-assistant ${embedded ? 'ai-assistant--embedded' : ''}`}>
      ...
      <p className="panel-hint panel-hint--compact">
        Règle métier en français → code Pandas.
      </p>
      <textarea
